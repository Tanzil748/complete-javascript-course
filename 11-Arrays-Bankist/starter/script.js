...
const displayMov = function (movements, sort = false) {
  containerMovements.innerHTML = ''; //clears out existing rows from container
...
btnLogin.addEventListener('click', function (e) {
  //Prevent form from submitting (makes login output stay in console)
  e.preventDefault();
...
const checkDogs = function (dogsJulia, dogsKate) {
  const correctJulia = [...dogsJulia].slice(1).slice(0, 2);
  const combinedData = correctJulia.concat(dogsKate);
...
//checks for one value (EQUALITY)
console.log(movements.includes(-130));

// checks for a condition (SUM)
...
//Numbers
console.log(movements);
console.log(movements.sort()); //does not work for numbers, only checks first digit
